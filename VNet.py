...
sheet3.write(0,0,"VirtualNetworkName")
sheet3.write(0,1,"Subscription")
sheet3.write(0,2,"Resource Group")
sheet3.write(0,3,"Location")
sheet3.write(0,4,"Address Space")
sheet3.write(0,5,"Subnets")
sheet3.write(0,6,"ApplicationName")
sheet3.write(0,7,"BusinessOwner")
sheet3.write(0,8,"Environment")
sheet3.write(0,9,"ServiceLine")
sheet3.write(0,10,"TechnologyOwner")
...
def run_example():
    ...
    try:
        ...
        for subscription in subscriptionClient.subscriptions.list():
            ...
            for group in groups:                
                for vm in network_client1.virtual_networks.list(group.name):
                    sub =[]
                    for m in range(len(vm.subnets)):
                        try:    
                            pub_id = vm.subnets[m].id.split('/')
                            sub.append(pub_id[10])
                            m= m+1
                        except: pass
                    sub_all= ", ".join(sub)
                    vms.append(vm.name)
                    ...
                    sheet3.write(k+1,4,vm.address_space.address_prefixes)
                    sheet3.write(k+1,5,sub_all)
                    try:
                            ...
                            sheet3.write(k+1,6,appname)
                            sheet3.write(k+1,7,busname)
                            sheet3.write(k+1,8,envname)
                            sheet3.write(k+1,9,servname)
                            sheet3.write(k+1,10,techname)
                    except: pass
                    k =k+1
        print("Total VMs Count : "+str(len(vms)))
        day = today_date.strftime("%B_%Y")
        file_name = 'Azure_VirtualNetwork_'+day+'.xls'
        wb.save(file_name)
    except CloudError:
    ...
    try:
        ...
        msg = MIMEMultipart()
        body_text = (
            "Attached herewith is the latest Azure VirtualNetwork Inventory \r\r\n"
                      "Total Number of VirtualNetwork: " + str(len(vms)) + " \r\r\n"
                     )
        #html = str(sys.argv[3])
        msg['Subject'] = "Azure Virtual Network Inventory List"
        msg['From'] = email_from
