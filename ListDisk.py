...
sheet3.write(0,0,"DiskName")
sheet3.write(0,1,"Subscription")
sheet3.write(0,2,"Resource Group")
sheet3.write(0,3,"Location")
sheet3.write(0,4,"Provisioning State")
sheet3.write(0,5,"Disk Size (GB)")
sheet3.write(0,6,"Created On")
sheet3.write(0,7,"SSD Type")
sheet3.write(0,8,"Attached To")
sheet3.write(0,9,"ApplicationName")
sheet3.write(0,10,"BusinessOwner")
sheet3.write(0,11,"Environment")
sheet3.write(0,12,"ServiceLine")
sheet3.write(0,13,"TechnologyOwner")
...
def run_example():
    ...
    try:
        ...
        for subscription in subscriptionClient.subscriptions.list():
            ...
            for group in groups:
                for item in compute_client1.disks.list_by_resource_group(group.name):
                    ...
                    sheet3.write(k+1,3,item.location)
                    sheet3.write(k+1,4,item.disk_state)
                    sheet3.write(k+1,5,item.disk_size_gb)
                    sheet3.write(k+1,6,item.time_created.date().strftime("%d-%B-%Y"))
                    sheet3.write(k+1,7,item.sku.tier)
                    try: 
                        ip_reference = item.managed_by.split('/')
                        vm_name = ip_reference[8]
                        sheet3.write(k+1,8,vm_name)
                    except: pass
                    try:
                            ...
                            sheet3.write(k+1,9,appname)
                            sheet3.write(k+1,10,busname)
                            sheet3.write(k+1,11,envname)
                            sheet3.write(k+1,12,servname)
                            sheet3.write(k+1,13,techname)
                    except: pass
                    k =k+1
        print("Total Disk Count : "+str(len(vms)))
        day = today_date.strftime("%B_%Y")
        file_name = 'Azure_Disk_'+day+'.xls'
        wb.save(file_name)
    except CloudError:
    ...
    try:
        ...
        msg = MIMEMultipart()
        body_text = (
            "Attached herewith is the latest Azure Disk Inventory \r\r\n"
                      "Total Number of Disks: " + str(len(vms)) + " \r\r\n"
                     )
        #html = str(sys.argv[3])
        msg['Subject'] = "Azure Disk Inventory List"
        msg['From'] = email_from
