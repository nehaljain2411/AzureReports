...
sheet3.write(0,6,"Enable Http2")
sheet3.write(0,7,"SKU Name")
sheet3.write(0,8,"Autoscale Min Capacity")
sheet3.write(0,9,"Autoscale Max Capacity")
...
def run_example():
    ...
    try:
        ...
        for subscription in subscriptionClient.subscriptions.list():
            ...
            for group in groups:                
                for vm in network_client1.application_gateways.list(group.name):
                    vms.append(vm)
                    ...
                    sheet3.write(k+1,4,vm.operational_state)
                    sheet3.write(k+1,6,vm.enable_http2)
                    
                    sheet3.write(k+1,7,vm.sku.name)
                    try: sheet3.write(k+1,8,vm.autoscale_configuration.min_capacity)
                    except: pass
                    try: sheet3.write(k+1,9,vm.autoscale_configuration.max_capacity)
                    except: pass
                    try:
                        pub_id = vm.frontend_ip_configurations[0].public_ip_address.id.split('/')
                        pub_name = pub_id[8]
                    ...
                    try:
                        sub_id = vm.gateway_ip_configurations[0].subnet.id.split('/')
                        sub_name = sub_id[10]
        ...
        file_name = 'Azure_ApplicationGateway_'+day+'.xls'
        wb.save(file_name)
    ...
    try:
        ...
        msg = MIMEMultipart()
        body_text = (
            "Attached herewith is the latest Azure Application Gateway Inventory \r\r\n"
                      "Total Number of Application Gateway: " + str(len(vms)) + " \r\r\n"
                     )
        #html = str(sys.argv[3])
        msg['Subject'] = "Azure Application Gateway Inventory List"
        msg['From'] = email_from
