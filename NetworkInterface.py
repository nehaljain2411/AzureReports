...
sheet3.write(0,0,"NetworkInterfaceName")
sheet3.write(0,1,"Subscription")
sheet3.write(0,2,"Resource Group")
sheet3.write(0,3,"Location")
sheet3.write(0,4,"Private IP")
sheet3.write(0,5,"Public IP Attached")
sheet3.write(0,6,"Security Group")
sheet3.write(0,7,"Virtual Machine")
sheet3.write(0,8,"Private IP Allocation")
sheet3.write(0,9,"Private IP Version")
...
def run_example():
    ...
    try:
        ...
        for subscription in subscriptionClient.subscriptions.list():
            ...
            for group in groups:                
                for vm in network_client1.network_interfaces.list(group.name):
                    vms.append(vm.name)
                    ...
                    sheet3.write(k+1,4,vm.ip_configurations[0].private_ip_address)
                    
                    try:    
                        pub_id = vm.ip_configurations[0].public_ip_address.id.split('/')
                        pub_name = pub_id[8]
                        vm2 = network_client1.public_ip_addresses.get(group.name, pub_name)
                        print(vm2.ip_address)
                        sheet3.write(k+1,5,pub_name+ "(" +vm2.ip_address +")")
                    except: pass
                    try:
                        sec_id = vm.network_security_group.id.split('/')
                        sec_name = sec_id[8]
                        sheet3.write(k+1,6,sec_name)
                    except: pass
                    try:
                        vm_id = vm.virtual_machine.id.split('/')
                        vm_name = vm_id[8]
                        sheet3.write(k+1,7,vm_name)
                    except: pass
                    sheet3.write(k+1,8,vm.ip_configurations[0].private_ip_allocation_method)
                    sheet3.write(k+1,9,vm.ip_configurations[0].private_ip_address_version)
                    try:
        ...
        file_name = 'Azure_NetworkInterfaces_'+day+'.xls'
        wb.save(file_name)
    except CloudError:
    ...
    try:
        ...
        msg = MIMEMultipart()
        body_text = (
            "Attached herewith is the latest Azure Network Interfaces Inventory \r\r\n"
                      "Total Number of Network Interfaces: " + str(len(vms)) + " \r\r\n"
                     )
        #html = str(sys.argv[3])
        msg['Subject'] = "Azure Network Interfaces Inventory List"
        msg['From'] = email_from
