...
sheet3.write(0,0,"App Service Name")
sheet3.write(0,1,"Subscription")
sheet3.write(0,2,"Resource Group")
sheet3.write(0,3,"Location")
sheet3.write(0,4,"State")
sheet3.write(0,5,"App Type")
sheet3.write(0,6,"Last Modified")
sheet3.write(0,7,"Reserved")
sheet3.write(0,8,"URL")
sheet3.write(0,9,"Outbound IP Address")
sheet3.write(0,10,"Container Size")
...
def run_example():
    ...
    try:
        ...
        for subscription in subscriptionClient.subscriptions.list():
            ...
            for group in groups:
                for vm in web_client1.web_apps.list_by_resource_group(group.name):
                    vms.append(vm.name)
                    ...
                    sheet3.write(k+1,4,vm.state)
                    sheet3.write(k+1,5,vm.kind)
                    sheet3.write(k+1,6,vm.last_modified_time_utc.date().strftime("%d-%B-%Y"))
                    try: sheet3.write(k+1,7,vm.reserved)
                    except: pass
                    sheet3.write(k+1,8,"https://"+vm.default_host_name)
                    try: sheet3.write(k+1,9,vm.outbound_ip_addresses)
                    except: pass
                    try: sheet3.write(k+1,10,vm.container_size)
                    except: pass
                    try:
        ...
        file_name = 'Azure_AppService_'+day+'.xls'
        wb.save(file_name)
    except CloudError:
    ...
    try:
        ...
        msg = MIMEMultipart()
        body_text = (
            "Attached herewith is the latest Azure AppService Inventory \r\r\n"
                      "Total Number of AppService: " + str(len(vms)) + " \r\r\n"
                     )
        #html = str(sys.argv[3])
        msg['Subject'] = "Azure AppService Inventory List"
        msg['From'] = email_from
