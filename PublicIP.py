...
sheet3.write(0,0,"Public IP Name")
sheet3.write(0,1,"Subscription")
sheet3.write(0,2,"Resource Group")
sheet3.write(0,3,"Location")
sheet3.write(0,4,"Public IP")
sheet3.write(0,5,"Allocation Mathod")
sheet3.write(0,6,"IP Address Version")
sheet3.write(0,7,"SKU Name")
sheet3.write(0,8,"Associated To Type")
sheet3.write(0,9,"Associated To Name")
sheet3.write(0,10,"Network Interface")
...
def run_example():
    ...
    try:
        ...
        for subscription in subscriptionClient.subscriptions.list():
            ...
            for group in groups:
                for vm in network_client1.public_ip_addresses.list(group.name):
                    pip_id= vm.id.split('/')
                    rg_from_pip = pip_id[4].lower()
                    pip_name = pip_id[-1]
                    for vm2 in network_client1.load_balancers.list(group.name):
                        try:
                            if (vm.id == vm2.frontend_ip_configurations[0].public_ip_address.id):
                                type1 = vm2.type.split('/')
                                sheet3.write(k+1,8,type1[1])
                                sheet3.write(k+1,9,vm2.name)
                            else: pass
                        except: pass                   
                    for vm1 in compute_client1.virtual_machines.list(group.name):
                        try:
                            ni_reference = vm1.network_profile.network_interfaces[0]
                            ...
                            if (vm.id == public_ip.id):
                                type1 = vm1.type.split('/')
                                sheet3.write(k+1,8,type1[1])
                                sheet3.write(k+1,9,vm1.name)
                            else: pass
                        except: pass
                            
                    for vm3 in network_client1.network_interfaces.list(group.name):
                        try:
                            if (vm.id == vm3.ip_configurations[0].public_ip_address.id):
                                type1 = vm3.type.split('/')
                                sheet3.write(k+1,10,vm3.name)
                            else: pass
                        except: pass
                    for vm4 in network_client1.virtual_network_gateways.list(group.name):
                        try:
                            if (vm.id == vm4.ip_configurations[0].public_ip_address.id):
                                type1 = vm4.type.split('/')
                                sheet3.write(k+1,8,type1[1])
                                sheet3.write(k+1,9,vm4.name)
                            else: pass
                        except: pass
                    for vm5 in network_client1.application_gateways.list(group.name):
                        try:
                            if (vm.id == vm5.frontend_ip_configurations[0].public_ip_address.id):
                                type1 = vm5.type.split('/')
                                sheet3.write(k+1,8,type1[1])
                                sheet3.write(k+1,9,vm5.name)
                            else: pass
                        except: pass 
                    vms.append(vm.name)
                    ...
                    sheet3.write(k+1,4,vm.ip_address)
                    sheet3.write(k+1,5,vm.public_ip_allocation_method)
                    sheet3.write(k+1,6,vm.public_ip_address_version)
        ...
        file_name = 'Azure_PublicIP_'+day+'.xls'
        wb.save(file_name)
    ...
    try:
        ...
        msg = MIMEMultipart()
        body_text = (
            "Attached herewith is the latest Azure Public IP Inventory \r\r\n"
                      "Total Number of PublicIP: " + str(len(vms)) + " \r\r\n"
                     )
        #html = str(sys.argv[3])
        msg['Subject'] = "Azure Public IP Inventory List"
        msg['From'] = email_from
